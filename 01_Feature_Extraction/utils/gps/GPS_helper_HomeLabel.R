...
#' Returns the mean point of the home cluster, i.e. the cluster where the user is most often at the given time +- the time tolerance.
...
#' findHome() returns the mean point of the home cluster
#' by Quay Au (Stachl et al., 2020) and adapted by Fiona Kunz & Ramona Schoedel


findHome = function(sensing.data, eps = 30, minPts = 3, time = "04:00:00", timeTolerance = 1*60*60, randomSample = 5000){
  ...
  # scan for most often visited cluster during these times and identify mean point of home cluster
    ...
    if (nrow(sensing_GPS) == 0){
      print(paste("User", unique(sensing.data$user_id)[!is.na(unique(sensing_GPS$user_id))], "has no sufficient GPS data in given time frame."))
